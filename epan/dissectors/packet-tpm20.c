...
static int hf_session_attribs = -1;
...
static void add_item(proto_tree *tree, int hf, tvbuff_t *tvb,
		     gint *offset, gint len, const guint encoding);
...
static void add_item(proto_tree *tree, int hf, tvbuff_t *tvb,
	gint *offset, gint len, const guint encoding)
{
	proto_tree_add_item(tree, hf, tvb, *offset, len, encoding);
	*offset += len;
}

static void
dissect_tpm20_platform_command(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *tree)
{
	gint offset = 0;
	...
	add_item(tree, hf_tpm20_platform_cmd, tvb, &offset, 4, ENC_BIG_ENDIAN);
	response_size = FALSE;
}

static void
dissect_auth_common(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *auth, proto_tree *tree UNUSED, gint *offset)
{
	guint16 nonce_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(auth, hf_session_nonce_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(auth, hf_session_nonce, tvb, offset, nonce_size, ENC_NA);
	guint8 attrib = tvb_get_guint8(tvb, *offset);
	proto_item *attr_it = proto_tree_add_uint_format(auth, hf_session_attribs, tvb, *offset, 1,
							attrib, "Session attributes 0x%x", attrib);
	proto_tree *attrib_tr = proto_item_add_subtree(attr_it, ett_tpm_attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_cont, tvb, *offset, 1, attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_auditex, tvb, *offset, 1, attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_auditreset, tvb, *offset, 1, attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_res, tvb, *offset, 1, attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_decrypt, tvb, *offset, 1, attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_encrypt, tvb, *offset, 1, attrib);
	proto_tree_add_boolean(attrib_tr, hf_session_attribs_audit, tvb, *offset, 1, attrib);
	*offset += 1;
	guint16 auth_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(auth, hf_session_auth_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(auth, hf_session_auth, tvb, offset, auth_size, ENC_NA);
}
static void
...
dissect_auth_command(tvbuff_t *tvb, packet_info *pinfo, proto_tree *auth,
		proto_tree *tree, gint *offset)
{
	guint32 auth_area_size = tvb_get_guint32(tvb, *offset, ENC_BIG_ENDIAN);
	...
	add_item(auth, hf_auth_area_size, tvb, offset, 4, ENC_BIG_ENDIAN);

	while (auth_area_size) {
		guint32 size;
		add_item(auth, hf_tpmi_sh_auth_session, tvb, offset, 4, ENC_BIG_ENDIAN);
		auth_area_size -= 4;
		size = *offset;
		dissect_auth_common(tvb, pinfo, auth, tree, offset);
...
static void
dissect_startup(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *header, proto_tree *tree UNUSED, gint *offset)
{
	add_item(header, hf_tpm20_startup_type, tvb, offset, 2, ENC_BIG_ENDIAN);
}

#define TPM_ALG_NULL 0x0010
static void
dissect_start_auth_session(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *header UNUSED, proto_tree *tree, gint *offset)
{
	guint16 nonce_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_session_nonce_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_session_nonce, tvb, offset, nonce_size, ENC_NA);

	guint16 encrypted = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_encrypted_secret_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_encrypted_secret, tvb, offset, encrypted, ENC_NA);
	add_item(tree, hf_session_type, tvb, offset, 1, ENC_NA);
	guint16 sym_alg = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);

	add_item(tree, hf_alg_sym, tvb, offset, 2, ENC_BIG_ENDIAN);
	if (sym_alg != TPM_ALG_NULL) {
		add_item(tree, hf_alg_sym_keybits, tvb, offset, 2, ENC_BIG_ENDIAN);
		add_item(tree, hf_alg_sym_mode, tvb, offset, 2, ENC_BIG_ENDIAN);
	}
	add_item(tree, hf_alg_hash, tvb, offset, 2, ENC_NA);
}

static void
dissect_create_primary(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *header UNUSED, proto_tree *tree, gint *offset)
{
	guint16 sensitive_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_sensitive_crate_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_sensitive_crate, tvb, offset, sensitive_size, ENC_NA);

	guint16 pub_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_pub_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_pub, tvb, offset, pub_size, ENC_NA);

	guint16 data_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_data_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_data, tvb, offset, data_size, ENC_NA);

	/*TODO TPML_PCR_SELECTION */
}

static void
dissect_create_loaded(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *header UNUSED, proto_tree *tree, gint *offset)
{
	guint16 sensitive_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_sensitive_crate_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	...
	guint16 template_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_template_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_template, tvb, offset, template_size, ENC_NA);
}
	...
	proto_tree *tree)
{
	...
	add_item(header, hf_tpm20_tag, tvb, &offset, 2, ENC_BIG_ENDIAN);
	add_item(header, hf_tpm20_size, tvb, &offset, 4, ENC_BIG_ENDIAN);
	add_item(header, hf_tpm20_cc, tvb, &offset, 4, ENC_BIG_ENDIAN);
	...
	if (handl_map.num_req_handles) {
		...
		for (i = 0; i < handl_map.num_req_handles; i++)
			add_item(hndl_tree, *handl_map.req_pd[i], tvb, &offset,
				 4, ENC_BIG_ENDIAN);
	}
	if (tag == 0x8002) {
...
#define RESP_SIZE 2
static void
dissect_tpm20_platform_response(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *tree)
{
	gint offset = 0;
	...
	if (entry->resp_type == RESP_SIZE) {
		...
		add_item(tree, hf_tpm20_platform_resp_size, tvb, &offset, 4, ENC_BIG_ENDIAN);
	} else {
		...
		add_item(tree, hf_tpm20_platform_resp_code, tvb, &offset, 4, ENC_BIG_ENDIAN);
	}
}

static void
dissect_start_auth_session_resp(tvbuff_t *tvb, packet_info *pinfo UNUSED, proto_tree *tree,
	gint *offset, guint32 param_size UNUSED)
{
	guint16 nonce_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_session_nonce_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_session_nonce, tvb, offset, nonce_size, ENC_NA);
...
static void
dissect_create_primary_resp(tvbuff_t *tvb, packet_info *pinfo UNUSED, proto_tree *tree,
	gint *offset, guint32 param_size UNUSED)
{
	guint16 pub_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_pub_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	...
	guint16 creation_data_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_creation_data_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_creation_data, tvb, offset, creation_data_size, ENC_NA);

	guint16 digest_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_digest_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_digest, tvb, offset, digest_size, ENC_NA);

	/* TODO TPMT_TK_CREATION; */

	guint16 name_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_name_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_name, tvb, offset, name_size, ENC_NA);
}

static void
dissect_create_loaded_resp(tvbuff_t *tvb, packet_info *pinfo UNUSED, proto_tree *tree,
	gint *offset, guint32 param_size UNUSED)
{
	guint16 priv_size = tvb_get_guint16(tvb, *offset, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_priv_size, tvb, offset, 2, ENC_BIG_ENDIAN);
	add_item(tree, hf_tpm_priv, tvb, offset, priv_size, ENC_NA);
...
static void
dissect_response(tvbuff_t *tvb, packet_info *pinfo UNUSED, proto_tree *tree,
	gint *offset, guint32 param_size)
{
	...
	default:
		...
		add_item(tree, hf_params, tvb, offset, param_size, ENC_NA);
	}
}

static void
dissect_tpm20_tpm_response(tvbuff_t *tvb, packet_info *pinfo UNUSED,
	proto_tree *tree)
{
	gint offset = 0;
	struct num_handles handl_map = { 0 };
	...
	add_item(header, hf_tpm20_resp_tag, tvb, &offset, 2, ENC_BIG_ENDIAN);
	add_item(header, hf_tpm20_resp_size, tvb, &offset, 4, ENC_BIG_ENDIAN);
	add_item(header, hf_tpm20_resp_code, tvb, &offset, 4, ENC_BIG_ENDIAN);
	/* if response code says error stop now */
	if (rc)
		return;

	/* find a correspondinf request */
	tpm_entry *entry = (tpm_entry *)wmem_tree_lookup32(cmd_tree, pinfo->num);
	DISSECTOR_ASSERT(entry != NULL);
	...
	if (handl_map.num_resp_handles) {
		...
		for (i = 0; i < handl_map.num_resp_handles; i++)
			add_item(hndl_tree, *handl_map.resp_pd[i], tvb, &offset,
				 4, ENC_BIG_ENDIAN);
	}
	...
	if (tag == 0x8002) {
		...
		add_item(tree, hf_resp_param_size, tvb, &offset, 4, ENC_BIG_ENDIAN);
...
static hf_register_info hf[] = {
	...
	{ "AUTH NONCE", "tpm.auth_nonce", FT_BYTES, BASE_ALLOW_ZERO | BASE_NONE, NULL,
	   0x0, NULL, HFILL }},
	{ &hf_session_attribs,
	{ "AUTH ATTRIBS", "tpm.auth_attribs", FT_UINT8, BASE_HEX, NULL,
	   0x0, NULL, HFILL }},
